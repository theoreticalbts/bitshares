...
program_options::variables_map parse_option_variables(int argc, char** argv)
{
   ...
   option_config.add_options()
         ...
         ("stop-at-block", program_options::value<uint32_t>(), "stop at given block index")
                                          ...
                                          bool sync_mode)
{
   try
   {
	  ...
	  while( (this->_debug_stop_block_num >= 1) &&
	         (block.block_num >= this->_debug_stop_block_num) )
	  {
...
void client::configure_from_command_line(int argc, char** argv)
{
   ...
   if( option_variables.count("stop-at-block") )
      my->_debug_stop_block_num = option_variables["stop-at-block"].as<uint32_t>();
